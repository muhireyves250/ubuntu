...
const TABS = ["Summary", "ANC Progress", "Timeline", "Visits"] as const;
type Tab = (typeof TABS)[number];

function VisitsSection({ pregnancy, visits }: { pregnancy: Pregnancy; visits: ReturnType<typeof useVisitsForPregnancy> }) {
...
export function PastPregnancyDetail({ pregnancy }: { pregnancy: Pregnancy }) {
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      {tab === "ANC Progress" && <VisitHistoryTab pregnancy={pregnancy} visits={visits} readOnly />}
      {tab === "Timeline" && <PregnancyTimeline visits={visits} referrals={referrals} readOnly />}
      {tab === "Visits" && <VisitsSection pregnancy={pregnancy} visits={visits} />}
    </div>
  );
}
