...
import { IconClock, IconActivity, IconClipboard } from "@/components/dashboard/icons";
  ...
  return (
    <div className="flex flex-col gap-3 rounded-2xl border border-zinc-200 bg-white p-4 dark:border-zinc-800 dark:bg-zinc-900">
      <div className="flex items-center gap-2">
        <span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-teal-50 text-teal-700 dark:bg-teal-950/40 dark:text-teal-400">
          {icon}
        </span>
...
export function PastPregnancyDetail({ pregnancy }: { pregnancy: Pregnancy }) {
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <SectionCard icon={<IconClock className="h-4 w-4" />} title="Antenatal Care Progress">
        <VisitHistoryTab pregnancy={pregnancy} visits={visits} readOnly />
      </SectionCard>

      <SectionCard icon={<IconActivity className="h-4 w-4" />} title="Pregnancy Timeline">
        <PregnancyTimeline visits={visits} referrals={referrals} readOnly />
      </SectionCard>

      <SectionCard
        icon={<IconClipboard className="h-4 w-4" />}
        title={`Visits during Pregnancy #${pregnancy.pregnancyNumber}`}
      >
        {visits.length === 0 ? (
          <p className="rounded-xl border border-zinc-200 bg-zinc-50 px-4 py-6 text-center text-sm text-zinc-500 dark:border-zinc-800 dark:bg-zinc-900 dark:text-zinc-400">
            No visits recorded for this pregnancy.
          </p>
        ) : (
